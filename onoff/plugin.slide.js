...
(function(){
	...
	window.PluginSlide = (function(){
		...
		var Slide = function(param){
			...
			// 1移動当たりに移動枚数
			this.perPage = 'perPage' in param ? parseInt(param.perPage) : 1;

			// スライド幅
			this.slideW = 'itemWidth' in param ? parseInt(param.itemWidth) : this.items.eq(0).outerWidth(false)*this.perPage;
			...
			if('itemGap' in param){
				...
				this.itemGap = parseFloat(this.items.eq(0).css('margin-right')) + parseFloat(this.items.eq(0).css('margin-left'));
			}
			...
			// 不正値チェック
			this.current = Shared.util.clamp(this.current, 0, Math.ceil(this.items.length/this.perPage)-1);
			...
			if(this.inds.get(0)){
				if(this.inds.length === this.items.length){
					...
					if(this.inds.children('*').length !== this.items.length){
						...
						for(var i=0,len=Math.ceil(this.items.length/this.perPage); i<len; i++){
			...
			if(this.type === this.SLIDE_TYPE_MOVE){
				...
				Shared.util.addResizeListener(function(w, h){
					...
					this.totalW = (this.slideW + this.itemGap) * Math.ceil(this.items.length/this.perPage);

					if(this.loop){
						this.slide.css({width:(this.slideW + this.itemGap) * Math.ceil((this.items.length+clones.length)/this.perPage)});
					}else{
						this.slide.css({width:(this.slideW + this.itemGap) * Math.ceil(this.items.length/this.perPage)});
					}
		...
		function setPosition(){
			...
			// ループの場合、クローンした画面分ずらす
			if(this.loop){
				var x = (this.slideX % this.totalW - this.totalW)%this.totalW - (this.slideW + this.itemGap)*Math.ceil(this.cloneNum/this.perPage);
			}else{
				var x = this.slideX;
			}
		...
		function beforeChange(first){
			...
			if(!this.loop){
				...
				if(this.getCurrent() === Math.ceil(this.items.length/this.perPage)-1){
		...
		function setFlick(){
			...
			this.slide.on('touchend mouseup', 'a', (function(self){
				return function(e){
					if(Math.abs(moved) > 5) noJump = true;
				};
			})(this));
		...
		Slide.prototype.change = function(index, noCheck, byTimer){
			...
			if(!this.loop){
				index = Shared.util.clamp(index, 0, Math.ceil(this.items.length/this.perPage)-1);
			}
		...
		Slide.prototype.getCurrent = function(){
			var length = Math.ceil(this.items.length/this.perPage);
			return (length +  this.current%length)%length;
		};
